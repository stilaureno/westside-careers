...
export default function ApplicantDetailPage({ params }: { params: Promise<{ referenceNo: string }> }) {
  const [referenceNo] = useState(() => {
    // In a real Next.js 15 app we'd await params, but for simplicity:
    return typeof window !== 'undefined' ? window.location.pathname.split('/').pop() : '';
  });
  ...
  useEffect(() => {
    (async () => {
      ...
      const res = await getApplicant(ref, '');
      setData(res.data);
      setLoading(false);
    })();
  }, []);
